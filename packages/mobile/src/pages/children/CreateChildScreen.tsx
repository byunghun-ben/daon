...
import {
  ...
  OnboardingCreateChildScreenProps,
} from "../../shared/types/navigation";

type Props = CreateChildScreenProps | OnboardingCreateChildScreenProps;

export default function CreateChildScreen({ navigation, route }: Props) {
  // Zustand 스토어에서 complete 함수 가져오기
  const completeOnboarding = useOnboardingStore((state) => state.complete);

  // TanStack Query 훅 사용
  const { data: childrenData, isLoading: isCheckingChildren } =
    useChildrenQuery();
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}>
          <Text style={styles.title}>
            {isFirstChild ? "아이 프로필 만들기" : "새 아이 프로필"}
          </Text>
